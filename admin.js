// admin.js (FINAL AND 100% CORRECTED CODE)
        ...
        db.ref(`orders/${orderId}`).update({ status: 'Completed' })
            .then(() => {
                ...
                if (statusElement) {
                    ...
                    e.target.remove(); // Button ko gayab kar dein
                }
            })
    ...
    filteredProducts.forEach(product => {
        ...
        productCard.innerHTML = `
            ...
            <div class="product-list-actions">
                <div class="stock-toggle"><label class="switch-small"><input type="checkbox" class="stock-status-checkbox" data-id="${product.key}" ${isInStock ? 'checked' : ''}><span class="slider-small round"></span></label><span class="stock-label">${isInStock ? 'In Stock' : 'Out of Stock'}</span></div>
                <div class="product-buttons"><button class="admin-btn-edit" data-id="${product.key}">Edit</button><button class="admin-btn-delete" data-id="${product.key}">Delete</button></div>
            </div>
        `;
        ...
        if (editingId) {
            ...
            if (productsSnapshot.exists()) {
                ...
                if (!isNaN(parseInt(numberPart))) {
                    lastProductNumber = parseInt(numberPart);
                }
            }
            const newProductNumber = lastProductNumber + 1;
            const newProductKey = `product${newProductNumber}`;
            const productNameWithId = `(P${newProductNumber}) ${name}`;

            await db.ref(`products/${newProductKey}`).set({ 
...
productListDiv.addEventListener('click', (e) => {
    ...
    if (target.classList.contains('admin-btn-edit')) {
        ...
        if (productToEdit) {
            document.getElementById('productName').value = productToEdit.name.replace(/\(P\d+\)\s/, ''); // Remove (PXXX) part
